...
export default function PeerMessages() {
  ...
  const { data: list = [] } = useQuery({
    ...
    queryFn: peerMessageRepo.conversations,
  });
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.pearl }}>
      <ScreenHeader
        title="Praticiens"
        rightAction={
          <Pressable onPress={() => router.push('/peer-messages/new' as any)} style={styles.plusBtn}>
      ...
      >
        <View style={{ paddingHorizontal: 20, paddingTop: 4 }}>
          <Text style={styles.sub}>Échangez avec d'autres praticiens de la communauté.</Text>
          <Input
          ...
          {filtered.length === 0 ? (
            <View style={styles.emptyWrap}>
              ...
              <Text style={styles.emptyBody}>
                Démarrez une conversation avec un autre praticien via le bouton +.
              </Text>
            </View>
          ) : (
            filtered.map((c) => (
              <Pressable
                key={c.id}
                style={styles.row}
                onPress={() => router.push(`/peer-messages/${c.id}` as any)}
              >
...
const styles = StyleSheet.create({
  ...
  sub: { ...typography.small, lineHeight: 19 },
  row: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 12,
